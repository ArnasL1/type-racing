...
$stmt = $pdo->prepare("
    SELECT s.*, sentences.text
    FROM scores s
    LEFT JOIN sentences ON s.sentence_id = sentences.id
    WHERE s.created_at BETWEEN :today_start AND :today_end
    ORDER BY s.mistakes ASC, s.time_taken ASC, s.created_at DESC
    LIMIT 10
");
                        ...
                        <tbody>
                            <?php foreach ($scores as $index => $score): ?>
                                ...
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="border-b border-slate-100 px-4 py-3">
                                        <?php echo number_format($score["time_taken"] / 1000, 2); ?>s
                                    </td>
